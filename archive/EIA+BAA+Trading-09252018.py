...
df_trade_all_stack_2016_resamp_stack.set_index('Datetime',inplace = True)
...
df_trade_all_stack_2016_resamp_stack['Exporting BAA'], df_trade_all_stack_2016_resamp_stack['Importing BAA'] = df_trade_all_stack_2016_resamp_stack['BAAs'].str.split('-', 1).str 
df_trade_all_stack_2016_resamp_stack['Exporting BAA_1'] = df_trade_all_stack_2016_resamp_stack['Exporting BAA'].str[4:] 
df_trade_all_stack_2016_resamp_stack['Importing BAA_1'] = df_trade_all_stack_2016_resamp_stack['Importing BAA'].str[0:-5] 
df_trade_all_stack_2016_resamp_stack['Transacting BAAs'] = df_trade_all_stack_2016_resamp_stack['Exporting BAA_1'] + '-' + df_trade_all_stack_2016_resamp_stack['Importing BAA_1']


#Take the absolute value of all values in the Exchange column and add as a new column Exchange_abs
df_trade_all_stack_2016_resamp_stack['Exchange_abs'] = df_trade_all_stack_2016_resamp_stack['Exchange'].abs()

#Check that the min value in the Exchange_abs column is now 0 
df_trade_all_stack_2016_resamp_stack['Exchange_abs'].min()
...
df_trade_sum_imp = df_trade_all_stack_2016_resamp_stack.groupby(['Importing BAA_1', 'Exporting BAA_1','Transacting BAAs'], as_index=False)[['Exchange', 'Exchange_abs']].sum()
df_trade_sum_exp = df_trade_all_stack_2016_resamp_stack.groupby(['Exporting BAA_1', 'Importing BAA_1', 'Transacting BAAs'], as_index=False)[['Exchange', 'Exchange_abs']].sum()
df_trade_sum_imp.columns = ['Imp - Importing BAA', 'Imp - Exporting BAA','Imp - Transacting BAAs', 'Imp - Exchange', 'Imp - Exchange_abs']
df_trade_sum_exp.columns = ['Exp - Exporting BAA', 'Exp - Importing BAA','Exp - Transacting BAAs', 'Exp - Exchange', 'Exp - Exchange_abs']

#Combine two grouped tables for comparison for exchange values
df_concat_trade = pd.concat([df_trade_sum_exp,df_trade_sum_imp], axis = 1)

#Calculate the difference in exchange values
df_concat_trade['Delta'] = df_concat_trade['Exp - Exchange_abs'] - df_concat_trade['Imp - Exchange_abs']

#Calculate percent diff of exchange_abs values
df_concat_trade['Percent_Diff']=(df_concat_trade['Exp - Exchange_abs']/df_concat_trade['Imp - Exchange_abs'])-1

#Mean exchange value
df_concat_trade['Exchange_mean'] = df_concat_trade[['Exp - Exchange_abs', 'Imp - Exchange_abs']].mean(axis=1)

#Percent diff equations creats NaN where both values are 0, fill with 0
df_concat_trade.fillna(0, inplace = True)

#Final exchange value based on logic; if percent diff is less than 20%, take mean, if not take the positive value, which represents the exporter
df_concat_trade['Final_Exchange'] = np.where((df_concat_trade['Percent_Diff'].abs() < 0.2) & (df_concat_trade['Exp - Exchange'] > 0), df_concat_trade['Exchange_mean'], np.where(df_concat_trade['Exp - Exchange'] > 0, df_concat_trade['Exp - Exchange'], 0))
...
df_concat_trade_subset = df_concat_trade[['Exp - Exporting BAA', 'Exp - Importing BAA', 'Final_Exchange']]

df_concat_trade_subset.columns = ['Exporting_BAA', 'Importing_BAA', 'Amount']

df_trade_pivot = df_concat_trade_subset.pivot(index = 'Exporting_BAA', columns = 'Importing_BAA', values = 'Amount')

df_trade_pivot.to_csv('trade_pivot.csv')
df_trade_pivot.drop(['GRIS','WAUE'], axis = 1, inplace = True)
df_trade_pivot.drop(['GRIS','WAUE'], axis = 0, inplace = True)
...
#Find out what columns are inconsistent between df_net_gen_2016 and df_trade_pivot
df_net_gen_2016.shape

df = df_net_gen_2016.columns[~df_net_gen_2016.columns.isin(df_trade_pivot.columns)]

df2 = df_trade_pivot.columns[~df_trade_pivot.columns.isin(df_net_gen_2016.columns)]
...
df_net_gen_2016.drop(['GRIS','WAUE'], axis=1, inplace = True)
...
for i in range(72):
    c.append(x[i] - df_trade_pivot.sum(axis = 0).iloc[i])
...
#Import matrix to split T into distinct interconnections - i.e., prevent trading between eastern and western interconnects

Interconnect = pd.read_csv('Interconnect_Split.csv', index_col = 0)
Interconnect_mat = Interconnect.values

T_split = np.multiply(T, Interconnect_mat)

#Export x and c vectors to csv to check
np.savetxt("x_vector.csv", x_np, delimiter=",")
np.savetxt("c_vector.csv", c_np, delimiter=",")
np.savetxt("x_hat_matrix.csv", x_hat, delimiter=",")
df_net_gen_2016_sum.to_csv('p_vector.csv')
#Export trade pivot to csv to check
df_trade_pivot.to_csv('T_split.csv')

#Matrix trading maths (see Qu et al. 2018 ES&T paper)
x_hat_inv = np.linalg.inv(x_hat)

np.savetxt("x_hat_inv_matrix.csv", x_hat_inv, delimiter=",")
...
np.savetxt("c_hat_x_hat_inv_matrix.csv", c_hat_x_hat_inv, delimiter=",")

G_c = np.matmul(G, c_hat)
H = np.matmul(G,c_hat, x_hat_inv)

df_G = pd.DataFrame(G)
df_G.to_csv('G_matrix.csv')
df_B = pd.DataFrame(B)
df_B.to_csv('B_matrix.csv')
df = pd.DataFrame(H)
df.to_csv('H_matrix.csv')
